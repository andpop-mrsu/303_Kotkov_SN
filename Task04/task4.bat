...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "select movies.title, movies.year, (select count(rating) from ratings inner join users on (users.id = user_id) where (movie_id = movies.id) and (users.gender = 'female') and(rating >= 4.5)) as rating_number from ratings inner join movies on (movie_id = movies.id) inner join users on (user_id = users.id) where (users.gender = 'female') and (rating >= 4.5) and (movies.genres = 'Drama') and (movies.year > 2005);"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "select title, year, avg(ratings.rating) as average_rating, count(distinct ratings.user_id) as number_of_ratings from movies inner join ratings on ratings.movie_id = movies.id group by movies.title order by movies.year, movies.title limit 20;"
echo " "

echo 8. Определить самый распространенный жанр фильма и количество фильмов в этом жанре
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "select a.genre, a.number_of_movies from (select genres as genre, count(distinct title) as number_of_movies from movies group by genre) a inner join (select max(number_of_movies) as max_number_of_movies from (select genres as genre, count(distinct title) as number_of_movies from movies group by genre)) b on a.number_of_movies = b.max_number_of_movies;"
echo " "